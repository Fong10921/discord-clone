export const API_URLS = {
  GET_CURRENT_USER_DATA: 'api/users',

  USER_PROFILE: "api/settings/profile/userProfile",
  USER_PROFILE_PICTURE: "api/settings/profile/userProfile/profilePicture",
  USER_PROFILE_BANNER_COLOR: "api/settings/profile/userProfile/bannerColor",

  SERVER_PROFILE: "api/settings/profile/serverProfile",
  SERVER_PROFILE_PICTURE: "api/settings/profile/serverProfile/profilePicture",
  SERVER_PROFILE_BANNER_COLOR: "api/settings/profile/serverProfile/bannerColor",
};